...
class AboutPreferenceFragment : PreferenceFragmentCompat() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val prefEula = this.findPreference<Preference>("pref_open_eula")
        prefEula?.onPreferenceClickListener = Preference.OnPreferenceClickListener {
            AboutUtils.showWebViewDialog(requireActivity(), getString(R.string.pref_about_license_title), getString(R.string.about_privacy_link), "open_eula")
            true
        }
        ...
        easterEgg?.onPreferenceClickListener = CountClickListener(
            object : CountClickListener.CountClickListener {
                override fun onCountClick() {
                    Toast.makeText(requireContext(), getString(R.string.about_easter_egg_msg), Toast.LENGTH_SHORT).show()
                }
            }
        )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_share -> {
                val gPlayLink = "https://play.google.com/store/apps/details?id=" + requireContext().packageName
                ShareCompat.IntentBuilder(requireActivity())
                    .setType("text/plain")
                    .setText(String.format(getString(R.string.action_share_description), AboutUtils.getAppName((requireContext())), gPlayLink)) //.setHtmlText(body) //If you are using HTML in your body text
